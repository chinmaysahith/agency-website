...
>(({ customClass, ...rest }, ref) => (
  <div
    ref={ref}
    {...rest}
    className={`cs-card ${customClass ?? ""} ${rest.className ?? ""}`.trim()}
  />
));
  ...
  const rendered = childArr.map((child, i) =>
    isValidElement(child)
      ? cloneElement(child as React.ReactElement<Record<string, unknown>>, {
          key: i,
          ref: refs[i],
          style: { width, height, ...(typeof (child as React.ReactElement<Record<string, unknown>>).props.style === 'object' ? (child as React.ReactElement<Record<string, unknown>>).props.style : {}) },
